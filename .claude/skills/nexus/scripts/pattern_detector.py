#!/usr/bin/env python3
"""
NEXUS Pattern Detector - Main Orchestrator
Analyzes SOUL files to detect patterns and recommend skills.

This is a streamlined orchestrator that uses modular components:
- ConfigManager: Manages configuration
- SOULDataReader: Reads and parses SOUL files
...
class PatternDetectorMain:
    ...
    def __init__(self, config_path: Optional[str] = None, repo_root: str = "."):
        ...
        self.soul_reader = SOULDataReader(repo_root, self.config)
    ...
    def run_analysis(self, output_path: Optional[str] = None) -> Dict:
        ...
        self.logger.info("Starting NEXUS pattern detection analysis")

        # Step 1: Read SOUL data
        self.logger.info("Reading SOUL data...")
    ...
    def print_summary(self, report: Dict):
        ...
        print("\n" + "=" * 80)
        print("NEXUS PATTERN DETECTION - SUMMARY")
        print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="NEXUS Pattern Detector - Analyze SOUL files and recommend skills",
...
Examples:
  ...
  python pattern_detector.py --output report --format json
    ...
    parser.add_argument(
        "--config",
        help="Path to NEXUS configuration file (JSON)",
        type=str
    )
    ...
    parser.add_argument(
        ...
        action="version",
        version="NEXUS Pattern Detector v2.0.0"
    )
